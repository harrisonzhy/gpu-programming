...
void launch_tail_latency_fma(
    ...
    // `block` is how many threads you need (block-64 means 32 threads per warp is 2 warps per block, 
    //      and the max product of block dims is 1024)
    // Tradeoff between latency hiding/occupancy and per-thread resource use:
    //  - block size 1024 -> 2 blocks per SM
    //  - block size 256  -> 8 blocks per SM
    //  - block size 128  -> 16 blocks per SM
    // `grid` is number of blocks to cover the work

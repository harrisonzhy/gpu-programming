...
__global__ void mandelbrot_gpu_vector_multicore(
    ...
    /* your (GPU) code here... */
    uint64_t linear_idx = (blockIdx.y * gridDim.x + blockIdx.x) * (blockDim.y * blockDim.x) + (threadIdx.y * blockDim.x + threadIdx.x);
    uint64_t i = linear_idx / img_size;
    uint64_t j = linear_idx % img_size;
...
static constexpr int32_t num_blocks = 32;

__global__ void __launch_bounds__(1024, num_blocks) mandelbrot_gpu_vector_multicore_multithread_single_sm(
    uint32_t img_size,
    uint32_t max_iters,
    uint32_t *out /* pointer to GPU memory */
) {
    ...
    const uint32_t num_elements = img_size * img_size;

    for (int32_t lin = linear; lin < num_elements; lin += (blockDim.y * blockDim.x) * num_blocks) {    
...
void launch_mandelbrot_gpu_vector_multicore_multithread_full(
    ...
    mandelbrot_gpu_vector_multicore<<<142 /* L40S has 142 SMs */, n_warps * n_threads_per_warp>>>(img_size, max_iters, out);
}
